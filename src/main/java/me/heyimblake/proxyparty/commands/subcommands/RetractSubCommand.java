...
public class RetractSubCommand extends PartySubCommand {
    ...
    public void execute(ProxiedPlayer player, String[] args) {
        ...
        if (!party.getInvited().contains(targetUniqueId.toString())) {
            player.sendMessage(new ComponentBuilder(String.format("%s no ha sido invitado a tu party.", args[0])).color(ChatColor.RED).create()[0]);
        ...
        RedisProvider.getInstance().sendPlayerMessage(targetUniqueId, "PARTY_RETRACTED%" + ProxyParty.translatePrefix(player));
