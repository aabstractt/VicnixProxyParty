...
public class KickSubCommand extends PartySubCommand {
    ...
    public void execute(ProxiedPlayer player, String[] args) {
        ...
        if (party.getLeader().equals(targetUniqueId.toString()) || player.getUniqueId().equals(targetUniqueId)) {
            player.sendMessage(new ComponentBuilder("No puedes kickearte a ti mismo o al lider de la party.").color(ChatColor.RED).create()[0]);
        ...
        player.sendMessage(ProxyParty.translatePrefix(targetUniqueId) + ChatColor.YELLOW + " ha sido removido de tu party.");
